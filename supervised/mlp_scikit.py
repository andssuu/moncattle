...
if __name__ == "__main__":
    ...
    #                     hidden_layer_sizes=(9, 6), batch_size=24, learning_rate_init=0.3, max_iter=20, momentum=0.7, alpha=0.0001)
    clf = MLPClassifier(solver='sgd', activation='logistic',
                        hidden_layer_sizes=(9, 6), batch_size=24, learning_rate_init=0.3, max_iter=20, momentum=0, alpha=0)
    ...
    clf = MLPClassifier(solver='sgd', activation='logistic',
                        hidden_layer_sizes=(9, 6), batch_size=24, learning_rate_init=0.3, max_iter=20, momentum=0.1, alpha=0.001)
    scores = cross_val_score(clf, data, labels, cv=10)
    [print("{}-Fold: {:.2f}".format(k, score))
        for k, score in enumerate(scores, 1)]
    print("Acurácia média: {:.2f}".format(np.mean(scores)))
