...
if __name__ == "__main__":
    ...
    fig.canvas.set_window_title('plot gyroscope all')
    ...
    for i, base in enumerate(bases):
        ...
        axs[int(i >= 5)][i % 5].plot(
            range(data.shape[0]), data.iloc[:, 0], label='gyro_x')
        axs[int(i >= 5)][i % 5].plot(
            range(data.shape[0]), data.iloc[:, 1], label='gyro_y')
        axs[int(i >= 5)][i % 5].plot(
            range(data.shape[0]), data.iloc[:, 2], label='gyro_z')
